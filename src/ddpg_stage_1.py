...
# print("testing_cuda",torch.cuda.is_available())
#---Directory Path---#
...
class Critic(nn.Module):
    ...
    #     xa = self.fa1(action)
    #     print('#############################################################')
    #     print('Concatenating in Critic')
    #     print('#############################################################')        
    #     x = torch.cat((xs,xa), dim=1)
    #     print(x.shape)
    #     x = nn.ReLU()(x)
    #     x = nn.ReLU(self.fca1(x))
    #     vs = self.fca2(x)
    #     return vs
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        ########################################################################
        # print("in forward state.size() = ", state.size())
        ...
        ########################################################################
        # print("in forward self.fa2(x).shape = ", self.fa2(x).shape)
        ########################################################################        
...
class Trainer:
    ...
    def save_models(self, episode_count):
        torch.save(self.target_actor.state_dict(), dirPath +'/Models/' + world + '/' + str(episode_count)+ '_actor.pth')
        torch.save(self.target_critic.state_dict(), dirPath + '/Models/' + world + '/'+str(episode_count)+ '_critic.pth')
        print('****Models saved***')
        
    def load_models(self, episode):
        self.actor.load_state_dict(torch.load(dirPath + '/Models/' + world + '/'+str(episode)+ '_actor.pth'))
        self.critic.load_state_dict(torch.load(dirPath + '/Models/' + world + '/'+str(episode)+ '_critic.pth'))
        hard_update(self.target_actor, self.actor)
        hard_update(self.target_critic, self.critic)
        print('***Models load***')
